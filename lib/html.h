...
"canvas{width:100%%;height:100%}.txt{margin:4px;font-weight:bold}</style>"
"<script>"
...
"let h=c.height,w=c.width;let max=Math.max(...data),min=Math.min(...data);"
"for(let i=0;i<data.length;i++){let x=i*(w/LIM),y=h-(data[i]-min)/(max-min+0.1)*h;"
...
"document.getElementById('p').innerText=d.pres.toFixed(1);"
...
"<div class='txt'>Temperatura: <span id='t'>--</span> °C</div>"
"<canvas id='gtemp' class='graf' width='360' height='100'></canvas>"
"<div class='txt'>Umidade: <span id='h'>--</span> %</div>"
"<canvas id='ghum' class='graf' width='360' height='100'></canvas>"
"<div class='txt'>Pressão: <span id='p'>--</span> kPa</div>"
"<canvas id='gpres' class='graf' width='360' height='100'></canvas>"
"<div class='txt'>Altitude estimada: <span id='alt'>--</span> m</div>"
